...
function getString(year){
	var result=['<table  cellpadding="2em" cellspacing="0">'];
	for(var month=0;month<12;month++){
		var d=new Date(year,month,1);//每个月初
		var d1=new Date(d.getFullYear(),d.getMonth()+1,0);//月末
		var th='<tr bgcolor="darkred" style="color:white"><th><span class="narrow">周</span><span class="wide">星期</span>日</th><th><span class="narrow">周</span><span class="wide">星期</span>一</th><th><span class="narrow">周</span><span class="wide">星期</span>二</th><th><span class="narrow">周</span><span class="wide">星期</span>三</th><th><span class="narrow">周</span><span class="wide">星期</span>四</th><th><span class="narrow">周</span><span class="wide">星期</span>五</th><th><span class="narrow">周</span><span class="wide">星期</span>六</th></tr>';
		result.push('<tr>'+'<td class="month center noborder" colspan=7>'+d.getFullYear()+"年"+(d.getMonth()+1)+"月"+'</td></tr>');
		result.push(th);
		result.push("<tr>");
		for(var i=0;i<d.getDay();i++) result.push("<td"+(i==0?' class="left"':'')+"></td>");
		for(var i=d.getDate();i<=d1.getDate();i++){
			...
			var t=[];
			t.push("<div class='center day0'>"+i)
			...
			if(d3.getDay()==0) 
				classname.push("left");
			title.push('公历'+d3.getFullYear()+"年"+(d3.getMonth()+1)+"月"+d3.getDate()+"日，藏历"+z.value);
			if(ecl.value!=""){
				title.push(ecl.value)
				if(/日/.test(ecl.value)) 
					classname.push("solareclipse");
				else 
					classname.push("lunareclipse");
			}		

			result.push('<td'+(classname.length>0?" class=\""+classname.join(" ")+"\"":"")+
			" title=\""+title.join(",")+'">');
			result.push(t.join(""));
			result.push("</td>");
		}
	...
	result.push("</table>");
	return('<!DOCTYPE html>\n<html>\n<head>\n<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width, initial-scale=1" />\n<meta name="keywords" content="在线,查询,藏历,百年,万年历,月历,online tibetan calendar,'+d.getFullYear()+'">\n<link rel="stylesheet" href="calendar.css" />\n<title>'+
		d.getFullYear()+'年藏历月历</title>\n</head>\n<body>'+result.join("").replace(/<\/tr><tr>/g,"<\/tr>\n\t<tr>")+
			'\n</body>\n</html>');
...
for(var i=1952;i<2051;i++){
	console.log(i+" start");
	var s=getString(i);
	console.log(i+" ready");
	fs.writeFile(i+'.html',s,function(err){
		if (err) {
